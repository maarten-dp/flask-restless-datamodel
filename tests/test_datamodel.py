...
def exposed_method_model_app(app):
    ...
    sr.serialize.all.CLASSES = {
        k: v for k, v in sr.serialize.all.CLASSES.items() if 'date' in str(k)}
    sr.serialize.all.CLASSES_BY_NAME = {
        k: v for k, v in sr.serialize.all.CLASSES_BY_NAME.items() if 'date' in str(k)}
...
def test_call_exposed_method(exposed_method_model_app, client_maker):
    ...
    res = sr.loads(client.post(url, json=body).json()['payload'], fmt='msgpack')
